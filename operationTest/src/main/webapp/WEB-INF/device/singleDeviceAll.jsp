        	...
        	<div class="col-sm-12 col-md-12  main">
        		...
        		<div class="panel panel-primary imeiDetailsInfo">
					<div class="panel-heading">
						...
						<span class="lead">IMEI：<label id="imei"></label></span>
      			...
      			<div class="panel panel-default imeiSingle">
      			 ...
      			 <div class="panel panel-default imeiSingle track-device">
					<div class="panel-heading">历史轨迹
					 <div class="fnTips">
					   <i class="glyphicon glyphicon-question-sign"></i>
					   <div class="fnTipsInfo">了解用户每段驾驶的行车轨迹和驾驶行为</div>
					</div>
					</div>
					<div class="panel-body">
						<div class="modalDropdownSelectBox clearfix"  id="historytrave">
							<div class="dropdown pull-right">
                           	<button class="btn btn-default dropdown-toggle" type="button"  id="historytime">
									&nbsp;2016.06.25~2016.07.01
									<span class="caret"></span>
								</button> 								
 								<ul class="dropdown-menu timeSelect" role="menu" id="timeSelect3">
									<li class="clearfix" role="presentation">
										<p class="pull-left">时间：</p>
										<select class="form-control pull-left year2" name="history-year1" id="history-year1">
											<option value="2016">2016年</option>
											<option value="2015">2015年</option>
											<option value="2014">2014年</option>
										</select>
										<select class="form-control pull-left month2 " name="history-month1" id="history-month1" >
											<option value="12">12月</option>
											<option value="11">11月</option>
											<option value="10">10月</option>
											<option value="9">9月</option>
										...
										<select class="form-control pull-left day2" name="history-day1" id="history-day1">
											<option value="31">31日</option>
											<option value="30">30日</option>
											<option value="29">29日</option>
											<option value="28">28日</option>
									...
									<li class="clearfix" role="presentation">
										<button type="button" class="btn btn-primary" id="historytrave-confirm">确定</button>
						...
						<div class="echartsBox">
					        <div class="ecCharLoading" id="historytrave-ecCharLoading"></div>
							<div class="otherInfoBody" id="homeMap1">
				<%-- 				<div class="row"> 
									<div class="col-xs-12 col-sm-4 col-md-4 historyGj"
										data-toggle="modal" data-target="#hsitoryModal">
										<img class="img-responsive" src="${contextPath}/images/history.png">
										<div class="time">
											<p>2016-01-12 15:00:00 至 2016-01-12 16:00:00</p>
										</div>
									</div> 
      			...
      			<div class="panel panel-default imeiSingle">
					<div class="panel-heading">停车监控视频
					<div class="fnTips">
					   <i class="glyphicon glyphicon-question-sign"></i>
...
//----------------------------------------------------------------用户单个驾驶行为初始化
	...
	//---------------------------------------------------------用户历史轨迹初始化 ；
	//监听父类事件
	$("#homeMap1").click(function(event){
		var usertrack = '<%=request.getSession().getAttribute("contextPath1")%>';
		var idImg=$(event.target).attr("id");
		if(idImg=="homeMap1"||idImg=="0"||idImg=="1")
			{
			return;
			}
		var idImg=idImg.split("-");
		var imgid = idImg[1];
		...
		layer.open({
					...
					closeBtn : 1,
					area:["80%","95%"],
					content : usertrack+"track"
				  });
	});
	$.post("${contextPath}/controller/singleDiverTrave",
							{
								imei:imei,
								time1:dateTime,
							},
							function(data) {
								$("#historytrave-ecCharLoading").hide();
								if (data.length < 1) {
							
									var html="<div class=row id=1>"+".... 该用户当天没有行车轨迹，请选择其他日期"+"</div>";
									 $("#homeMap1").append(html);	
								}else{
									trackData = data;
									
									for(var i=0;i<Math.ceil(data.length/3);i++)
										{
										var html="<div class=row id="+i+">"+"</div>";
										 $("#homeMap1").append(html);	
										for(var k=1;k<=3;k++)
											{
											if((i*3+k)>data.length)
												{
												break;
												}
											
							  var html1="<div class='col-xs-12 col-sm-4 col-md-4 historyGj'"+" id=map"+(i*3+k)+
										" data-toggle=modal data-target=#hsitoryModal>"+
										"	<img  id=img-"+(i*3+k)+" class=img-responsive src=>"+
										"	<div class=time>"+
										"		<p id=p"+(i*3+k)+">2016-01-12 15:00:00 至 2016-01-12 16:00:00</p>"+
										"	</div>"+
									    "</div>";
										 $("#"+i).append(html1);
										  }
									...
									for (var k = 0; k < data.length; k++) {
										...
										var trackTime11 = BaseUtility.longConvertDate(trackTime1);
										var trackTime22 = BaseUtility.longConvertDate(trackTime2);
										
										var a=trackTime11+"到"+trackTime22;
										...
										var pt11;
									 	if (data1[0].type == 1)
											pt11 = wgs2bd(data1[0].lat,
													data1[0].lng);
										else {
											pt11 = gcj2bd(data1[0].lat,
													data1[0].lng);
										} 
										var center1;
										if ((data1.length % 2) == 0) {
										...
										var center=data1[center1];
										if (data1[center1].type == 1)
											center = wgs2bd(data1[center1].lat,
													data1[center1].lng);
										else {
											center = gcj2bd(data1[center1].lat,
													data1[center1].lng);
										} 
										...
										for (var i = 1; i < data1.length - 1; i++) {
											...
											point.push(new BMap.Point(pt00.lng,
													pt00.lat));
											paths = paths + pt00.lng + ","
													+ pt00.lat + ";"
										}
										//终点：
										var pt12=data1[data1.length - 1];
							/* 			if (data1[data1.length - 1].type == 1)
										...
										var src = "http://api.map.baidu.com/staticimage?width=490&height=300&center="
												+ center.lng
												+ ","
												+ center.lat
												+ "&zoom=16&paths="
...
$("#driverbehaviortime-confirm").click(function(event){
	...
	//--------------------------------------------------------------历史轨迹时间选择；	
	   $("#historytrave").click(function(event){
            $(this).find(".dropdown-menu").show();
        });
		
	$("#historytrave-confirm").click(function(event){
		
		var time1 = $("#history-year1 option:selected").val();
		var time2 = $("#history-month1 option:selected").val();
		var time3 = $("#history-day1 option:selected").val();
		var time4 = time1 + "." + time2 + "." + time3;	

		/* var time5 = $("#history-year2 option:selected").val();
		var time6 = $("#history-month2 option:selected").val();
		var time7 = $("#history-day2 option:selected").val();
		var time8 = time5 + "." + time6 + "." + time7;	 */
		var start;
		var end
		 var mb = BaseUtility.myBrowser();
		...
		//	var history = baseDate(end, start);	

			$("#historytime").text(time4);
			
			$("#historytrave-ecCharLoading").show();
			...
			$.post("${contextPath}/controller/singleDiverTrave", 
							...
							time1:time4,
								...
								function(data){
									...
									if (data.length < 1) {
										alert("该用户当天没有行车轨迹，请选择其他日期");
									}else{
										trackData = data;
										//删除子元素
										$("#homeMap1").children("div").remove();
										for(var i=0;i<Math.ceil(data.length/3);i++)
											{
											var html="<div class=row id="+i+">"+"</div>";
											 $("#homeMap1").append(html);	
								  ...
								  var html1="<div class='col-xs-12 col-sm-4 col-md-4 historyGj'"+" id=map"+(i*3+k)+
									...
									/* 			if (data1[i].type == 1)
												...
												point.push(new BMap.Point(pt00.clng,
														pt00.clat));
												paths = paths + pt00.clng + ","
														+ pt00.clat + ";"
											}
											//终点：
											var pt12=data1[data1.length - 1];
